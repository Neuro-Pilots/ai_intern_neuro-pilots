...
class LLMReasoner:
    ...
    def reason(
        ...
        # Add other supported providers as fallbacks
        ...
        for current_provider in providers_to_try:
            if not self.is_provider_configured(current_provider):
                continue

            try:
                # Temporarily override provider and model for the call
                original_provider = self.provider
                original_model = self.model

                self.provider = current_provider
                self.model = self._default_model_for_provider(current_provider)
        ...
        return LLMReasoningResult(
            ...
            prompt=prompt,
            success=False,
            error=f"All attempted providers failed. Last error: {last_error}",
        )
...
""".strip()
    ...
    def _reason_with_gemini(
        ...
        """
        Generate reasoning using Google Gemini Advanced.
        Implements:
        - Structured Output (Pydantic)
        - Google Search Grounding
        - Advanced Thinking (via model choice)
        """

        client = genai.Client(api_key=GEMINI_API_KEY)

        # We use the newest 3.8 flash model from the docs
        model_id = "gemini-3.8-flash"

        # Construct the request with Advanced Features
        # 1. Google Search Grounding
        # 2. Structured Output (mapping to ExperimentRecommendation)
        try:
            response = client.interactions.create(
                ...
                config=types.GenerateContentConfig(
                    ...
                    response_schema=ExperimentRecommendation.model_json_schema(),
                ),
            )

            # Extract the reasoning from the structured JSON output
            # Since we forced JSON, we return the string representation for the dashboard
            # but the metadata contains the structured data.
            reasoning_json = response.output_text or ""
            ...
            return reasoning_json, metadata

        except Exception as exc:
            # Fallback to basic text generation if structured output fails
            ...
            return (response.text or ""), {"provider": "gemini", "model": model_id, "structured": False}
